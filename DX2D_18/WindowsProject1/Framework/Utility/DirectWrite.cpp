...
DirectWrite::DirectWrite()
{
    ...
    assert(SUCCEEDED(Device::GetInstance()->SwapChain()->GetBuffer(0, __uuidof(IDXGISurface), (void**)dxgiSurface.GetAddressOf())));
...
void DirectWrite::RenderText(wstring text, RECT rect, float size, wstring fontPath, XMFLOAT4 color, DWRITE_FONT_WEIGHT weight, DWRITE_FONT_STYLE style, DWRITE_FONT_STRETCH stretch)
{
    ...
    if (findBrush == nullptr)
    {
        ...
        assert(SUCCEEDED(_dc->CreateSolidColorBrush(colorF, OUT brushDesc._brush.GetAddressOf())));
    ...
    if (findText == nullptr)
    {
        assert(SUCCEEDED(_writeFactory->CreateTextFormat(fontDesc._fontPath.c_str(), nullptr, fontDesc._weight, fontDesc._style, fontDesc._stretch,
